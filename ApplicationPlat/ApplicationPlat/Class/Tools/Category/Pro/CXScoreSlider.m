...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        backView.backgroundColor = ColorString(@"#DEE8F0");
        ...
        trackView.backgroundColor = DefaColor;
        ...
        [thumb setImage:[UIImage imageNamed:@"icon_slider_cycle"]];
...
- (void)setScore:(NSInteger)score {
    _score = score;
    //刷新视图
    [self reloadViewWithThumbCeneterX:score / 24.0 * self.bounds.size.width];
    //动画放大选择的效果
    [self endReloadViewWithThumbCeneterX:@(_score / 24.0 * self.bounds.size.width)];
}
...
- (void)reloadViewWithThumbCeneterX:(CGFloat)thumbCeneterX {
    ...
    if (_lastTransformLable) {
        [UIView animateWithDuration:0.25 animations:^{
            weakself.lastTransformLable.transform = CGAffineTransformIdentity;
        }];
    }
    
    [UIView animateWithDuration:0.1 animations:^{
        //更新轨道前景色
        weakself.trackView.width = thumbCeneterX;
        //更新滑块位置
        weakself.thumb.centerX = thumbCeneterX;
    }];
    
    //分数，四舍五入取整
    _score = round(thumbCeneterX / self.bounds.size.width * 24);
    if (_score < 1) {
        _score = 1;
    }
}
...
- (void)endReloadViewWithThumbCeneterX:(NSNumber *)thumbCeneterX {
    ...
    [UIView animateWithDuration:0.2 animations:^{
        //更新轨道前景色
        weakself.trackView.width = thumbCeneterX.floatValue;
        //更新滑块位置
        weakself.thumb.centerX = thumbCeneterX.floatValue;
    }completion:^(BOOL finished) {
        !self.reloadHandler?:self.reloadHandler(self,weakself.score);
        UILabel *la = (UILabel *)[self viewWithTag:(weakself.score + 2019)];
        weakself.lastTransformLable = la;
        CGFloat y = weakself.score%2 == 1?8:-10;
