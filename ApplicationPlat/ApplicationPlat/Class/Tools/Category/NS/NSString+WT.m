...
- (CGFloat)heightWithCustomFont:(UIFont *)font w:(CGFloat)w {
...
- (CGFloat)widthWithSize:(NSInteger)size h:(CGFloat)h {
    return [self boundingRectWithSize:CGSizeMake(MAXFLOAT, h) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{ NSFontAttributeName : [UIFont systemFontOfSize:size] } context:nil].size.width;
}
- (CGFloat)widthWithFont:(UIFont *)font h:(CGFloat)h {
...
- (BOOL)isValidateMobile{
    if (self.length !=0) {
        NSString *aimStr = [self stringByReplacingOccurrencesOfString:@" " withString:@""];
