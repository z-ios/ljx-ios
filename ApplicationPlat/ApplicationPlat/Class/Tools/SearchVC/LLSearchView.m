...
{
    if (!_hotSearchView) {
        self.hotSearchView = [self setViewWithOriginY:CGRectGetHeight(_searchHistoryView.frame) title:NSLocalizedString(@"typePopularSearches", nil) textArr:self.hotArray];
    }
...
- (UIView *)searchHistoryView
{
    if (!_searchHistoryView) {
        if (_historyArray.count > 0) {
            ...
            self.searchHistoryView = [self setViewWithOriginY:0 title:NSLocalizedString(@"typeRecentSearch", nil) textArr:self.historyArray];
        } else {
...
- (void)reloadHistoryArray:(NSMutableArray *)hisArray
{
    if (hisArray.count > 0) {
        [self.searchHistoryView removeFromSuperview];
        self.searchHistoryView = [self setViewWithOriginY:0 title:NSLocalizedString(@"typeRecentSearch", nil) textArr:hisArray];
        [self addSubview:self.searchHistoryView];
...
- (UIView *)setViewWithOriginY:(CGFloat)riginY title:(NSString *)title textArr:(NSMutableArray *)textArr
{
    ...
    if ([title isEqualToString:NSLocalizedString(@"typeRecentSearch", nil)]) {
    ...
    for (int i = 0; i < textArr.count; i++) {
        ...
        if (letfWidth + width + 15 > KScreenWidth) {
            if (y >= 130 && [title isEqualToString:NSLocalizedString(@"typeRecentSearch", nil)]) {
        ...
        if (i % 2 == 0 && [title isEqualToString:NSLocalizedString(@"typePopularSearches", nil)]) {
...
- (UIView *)setNoHistoryView
{
    ...
    titleL.text = NSLocalizedString(@"typeRecentSearch", nil);
    titleL.font = [UIFont systemFontOfSize:15];
    titleL.textColor = [UIColor blackColor];
    titleL.textAlignment = NSTextAlignmentLeft;
    
    UILabel *notextL = [[UILabel alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(titleL.frame) + 10, 100, 20)];
    notextL.text = NSLocalizedString(@"typeNoHistory", nil);
