...
#define DEFAULT_MAX_HEIGHT KScreenHeight/3*2
 ...
 */
+ (void)showUpdateAlertWithVersion:(NSString *)version Description:(NSString *)description Url:(NSString *)url
{
    SELUpdateAlert *updateAlert = [[SELUpdateAlert alloc]initVersion:version description:description Url:url];
    [[[UIApplication sharedApplication].windows lastObject] addSubview:updateAlert];
}

- (instancetype)initVersion:(NSString *)version description:(NSString *)description Url:(NSString *)url
{
    self = [super init];
    if (self) {
...
- (void)_setupUI
{
    ...
    [bgView borderRoundCornerRadius:5];
    ...
    updateView.layer.cornerRadius = 4.f;
