...
#define ALERT(msg) [[[UIAlertView alloc] initWithTitle:nil message:msg delegate:self cancelButtonTitle:@"好的" otherButtonTitles:nil] show]

@interface QRScanViewController () <AVCaptureMetadataOutputObjectsDelegate, CAAnimationDelegate, AVCaptureVideoDataOutputSampleBufferDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
{
...
- (void)initBaseUI
{
    CGFloat H = IS_IPHONE_Xx ? 47 : 27;
    
    UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
    btn.frame = CGRectMake(15, H, 40, 40);
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(Width/2-30, H, 60, 40)];
    ...
    UIButton *xcbtn = [UIButton buttonWithType:UIButtonTypeCustom];
    xcbtn.frame = CGRectMake(KScreenWidth - 40-15, H, 40, 40);
    [xcbtn setTitle:@"相册" forState:UIControlStateNormal];
    xcbtn.titleLabel.font    = [UIFont systemFontOfSize: 18];
    [xcbtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
...
- (void)xcbtnClick{
    ...
    imagrPicker.modalPresentationStyle = 0;
    ...
    [picker dismissViewControllerAnimated:YES completion:^{
        ...
        if (features.count >= 1) {
            ...
            if ([self.delegate respondsToSelector:@selector(qrScanResult:viewController:)]) {
                [self.delegate performSelector:@selector(qrScanResult:viewController:) withObject:scannedResult withObject:self];
            }
            //通过对话框的形式呈现
...
//        [alert show];
        [self alertVc:@"请在设置->隐私中允许该软件访问摄像头"];
