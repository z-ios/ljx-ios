...
- (void)setValue:(id)value forKey:(NSString *)key{
    ...
    if ([key isEqualToString:@"children"]) {
        NSMutableArray<BTStreets*> * childrenArray = [[NSMutableArray alloc] init];
        for (NSDictionary *dic in value) {
            BTStreets * children = [[BTStreets alloc]initWithDictionary:dic];
...
//        NSArray * temArray = [NSArray loadJsonOfBundlePathForResource:@"pca-code"];
        NSArray * temArray = [NSArray loadJsonOfBundlePathForResource:@"sojson"];
        ...
        self.selectedStreet = self.selectedArea.children.firstObject;
...
- (NSString *)description{
    return [NSString stringWithFormat:@"%@ %@ %@ %@", self.selectedProvince.name,self.selectedCitie.name,self.selectedArea.name,self.selectedStreet.name];
}
@end
