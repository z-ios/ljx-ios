...
//        CGSize textSize = [text sizeWithFont:FontSize constrainedToSize:CGSizeMake(maxlabel, maxlabel)];
        ...
        attrs[NSFontAttributeName] = FontSize;
        CGSize textSize = [text boundingRectWithSize:CGSizeMake(maxlabel, maxlabel) options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesFontLeading  |NSStringDrawingUsesLineFragmentOrigin attributes:attrs context:nil].size;
        ...
        {
            labelY = (self.backView.frame.size.height - labelHeight - sizeSpace - Height_StatusBar);
        }
        ...
        {
            [self performSelector:@selector(hiddenIToast) withObject:nil afterDelay:2.0];
        }
    }
}
