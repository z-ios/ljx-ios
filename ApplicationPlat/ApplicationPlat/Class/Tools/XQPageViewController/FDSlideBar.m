...
NSInteger const MenuButtonWidth = 90;
...
- (void)setItemSelectedBgColor:(UIColor *)itemSelectedBgColor
{
    ...
    for (FDSlideBarItem *item in _items) {
        [item setItemSelectBgColor:itemSelectedBgColor];
    }
}
- (void)setItemBgColor:(UIColor *)itemBgColor
{
    ...
    for (FDSlideBarItem *item in _items) {
        [item setItemBgColor:itemBgColor];
    }
}
...
- (void)setupItems {
    ...
    for (NSString *title in _itemsTitle) {
        ...
        item.frame = CGRectMake(itemX, 0, itemW + 20 * WidthScale, CGRectGetHeight(_scrollView.frame));
        ...
        if (_itemSelectedBgColor) {
            [item setItemSelectBgColor:_itemSelectedBgColor];
        }
        if (_itemBgColor) {
            [item setItemBgColor:_itemBgColor];
        }
...
//        item.backgroundColor = [UIColor blueColor];
        ...
        itemX = CGRectGetMaxX(item.frame) + 10 * WidthScale;
        item.layer.cornerRadius = item.height / 2;
    ...
    if (self.isUnifyWidth) {
        ...
        if (_showMenuButton) {
            ...
            inset.right += LAYOUTINSET + MenuButtonWidth * WidthScale;
        } else {
            inset.left += LAYOUTINSET;
    ...
    firstItem.backgroundColor = _itemSelectedBgColor;
    firstItem.layer.shadowOffset = CGSizeMake(0, 4);
    firstItem.layer.shadowColor = [UIColor colorWithHex:@"#2A3747" alpha:0.15].CGColor;
    firstItem.layer.shadowRadius = firstItem.height / 2;
...
- (void)scrollToVisibleItem:(FDSlideBarItem *)item {
    dispatch_async(dispatch_get_main_queue(), ^{
        [UIView animateWithDuration:0.25 delay:0 options:UIViewAnimationOptionCurveLinear | UIViewAnimationOptionBeginFromCurrentState animations:^{
            ...
            self.sliderView.frame = CGRectMake(CGRectGetMinX(item.frame), self.sliderView.frame.origin.y, CGRectGetWidth(item.frame), SLIDER_VIEW_HEIGHT);
